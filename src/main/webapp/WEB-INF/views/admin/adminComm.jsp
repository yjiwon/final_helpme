...
<script src="http://code.jquery.com/jquery-1.6.4.min.js"></script>
	<script type="text/javascript">
		...
		function deleteValue(){
			var url = "deleteBoard";    // Controller로 보내고자 하는 URL
		    ...
		    if (valueArr.length == 0){
				...
				$.ajax({
	                ...
	                success: function(jdata){
	                    if(jdata = 1) {
	                        alert("삭제 성공");
	                        location.replace("adminComm") //list 로 페이지 새로고침
	                    }
	                    else{
	                        alert("삭제 실패");
	                    }
			...
			<div class="row">
				...
				<div class="col-md-8"  style="float: none; margin:0 auto;">
					<div class="card">
						...
						<!--					<label><select name="bootstrap-data-table_length"
									...
									<tbody>
										<c:forEach items="${list}" var="list">
											<tr>
												<!-- 게시글 상세조회 -->
												<td>
												<input name="RowCheck" type="checkbox" value="${list.cBoardId}"/>
												</td>
												<td><a href= "${path}/board/view?cBoardId=${list.cBoardId}">
												${list.cBoardId}
												<td>${list.cBoardTitle}</td>
												<td>${list.userId}</td>
												<td><fmt:formatDate pattern="yyyy-MM-dd"
                                                value="${list.cRegdate}" /></td>
